...
export async function bsInitiateBroadcast(payload) {
  return new Promise((resolve, reject) => {
    const {userId, broadcastId, hostId, title, streamKind, seats, initiatedAt} =
      payload;
    appServerWS.emit(
      'core::broadcast-initiate',
      {
        userId,
        broadcastId,
        hostId,
...
export async function bsOngoingBroadcastList() {
  return new Promise((resolve, reject) => {
    appServerWS.emit('core::broadcast-ongoing-list', {}, async data => {
      if (data.error) {
        reject(data.error);
      }
      resolve(data.data);
...
export async function bsJoinAsBroadcaster(payload) {
  return new Promise((resolve, reject) => {
    ...
    appServerWS.emit(
      'core::broadcast-room-join-as-broadcaster',
      {
        broadcastId,
        userId,
        publisherId,
...
export async function bsSendComment(payload) {
  return new Promise((resolve, reject) => {
    const {broadcastId, userId, subscriberId, comment, chatText, kind, name} =
      payload;
    console.log('bsSendComment Params', payload);

    appServerWS.emit(
      'core::broadcast-room-send-comment',
      {
        broadcastId,
        userId,
        subscriberId,
...
export async function bsStopBroadcasting(payload) {
  return new Promise((resolve, reject) => {
    ...
    appServerWS.emit(
      'core::broadcast-room-stop-broadcasting',
      {
        broadcastId,
        userId,
      },
